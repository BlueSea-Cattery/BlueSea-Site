...
interface Achievement { id: string; url: string; caption: string | null; sortOrder: number; }
...
export default function AdminAchievementsPage() {
  ...
  const fetchItems = async () => {
    setItems(await (await fetch("/api/achievements")).json());
    setIsLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch("/api/achievements", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url, caption, sortOrder: items.length }) });
    setShowModal(false); setUrl(""); setCaption(""); fetchItems();
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Удалить?")) return;
    await fetch(`/api/achievements/${id}`, { method: "DELETE" }); fetchItems();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-serif text-ink-800">Достижения</h1>
        ...
        {items.map((item) => (
          <div key={item.id} className="admin-card p-2 group relative">
            <div className="relative aspect-square rounded-lg overflow-hidden">
              <Image src={getProxiedUrl(item.url)} alt={item.caption || ""} fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" />
              <div className="absolute inset-0 bg-ink-900/0 group-hover:bg-ink-900/40 transition-colors flex items-center justify-center">
                <button onClick={() => handleDelete(item.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-2 bg-red-500 rounded-lg text-white hover:bg-red-600 shadow-lg">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            {item.caption && <p className="text-xs text-ink-500 mt-2 px-1 truncate">{item.caption}</p>}
          </div>
        ))}
        {items.length === 0 && <div className="col-span-full text-center py-16 text-ink-400">Нет достижений</div>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-xl border border-black/5">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-serif text-ink-800">Добавить достижение</h2>
