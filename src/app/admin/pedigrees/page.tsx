...
interface PedigreeItem { id: string; name: string; photoUrl: string; }

export default function AdminPedigreesPage() {
  const [items, setItems] = useState<PedigreeItem[]>([]);
  ...
  const fetchItems = async () => {
    setItems(await (await fetch("/api/pedigrees")).json());
    setIsLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowModal(false); setName(""); setUrl(""); fetchItems();
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Удалить родословную?")) return;
    await fetch(`/api/pedigrees/${id}`, { method: "DELETE" }); fetchItems();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-serif text-ink-800">Родословные</h1>
        <button
        ...
        >
          <Plus size={16} /> Добавить родословную
        </button>
      </div>
        ...
        {items.map((item) => (
          <div key={item.id} className="admin-card p-2 group relative">
            <div className="relative aspect-square rounded-lg overflow-hidden">
              <Image src={getProxiedUrl(item.photoUrl)} alt={item.name} fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" />
              <div className="absolute inset-0 bg-ink-900/0 group-hover:bg-ink-900/40 transition-colors flex items-center justify-center">
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 transition-opacity p-2 bg-red-500 rounded-lg text-white hover:bg-red-600 shadow-lg"
                >
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            <p className="text-xs text-ink-600 font-medium mt-2 px-1 truncate">{item.name}</p>
          </div>
        ))}
        {items.length === 0 && !isLoading && (
          <div className="col-span-full text-center py-16 text-ink-400">Нет родословных</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-xl border border-black/5">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-serif text-ink-800">Добавить родословную</h2>
              <button onClick={() => setShowModal(false)} className="text-ink-400 hover:text-ink-700"><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm text-ink-500 mb-1">Название *</label>
                <input required value={name} onChange={(e) => setName(e.target.value)} className={inputStyle} placeholder="Например: Родословная помёт А" />
              </div>
              <div>
                ...
                {url && (
                  <div className="relative w-full aspect-video rounded-lg overflow-hidden mt-3 shadow-sm">
                    <Image src={getProxiedUrl(url)} alt="Preview" fill className="object-cover" sizes="400px" />
                  </div>
                )}
              </div>
